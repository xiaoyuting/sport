...
- (instancetype)init
{
    ...
    if (self) {
        ...
        NSArray * typeArr = @[@(JSHAREPlatformSinaWeibo),
          ...
          @(JSHAREPlatformQQ),
          @(JSHAREPlatformQzone)];
        NSArray * titleArr = @[@"新浪微博",@"微信好友",@"微信朋友圈",@"微信收藏",@"QQ好友",@"QQ空间"];
        NSArray * imageArr = @[@"weibo",@"wechat",@"wechat_moment",@"wechat_fav",@"qq",@"qzone"];
...
- (void)showWithContentType:(JSHAREMediaType)type {
  ...
  if (type == JSHAREEmoticon) {
    [self.currentContentSupportPlatform addObjectsFromArray:@[@(JSHAREPlatformWechatSession)]];
  }else if(type == JSHAREApp){
      [self.currentContentSupportPlatform addObjectsFromArray:@[@(JSHAREPlatformWechatSession),
                                                                @(JSHAREPlatformWechatTimeLine)]];
  }else if(type == JSHAREFile){
      [self.currentContentSupportPlatform addObjectsFromArray:@[@(JSHAREPlatformWechatSession),
                                                                @(JSHAREPlatformWechatFavourite),
                                                                ]];
  }else if (type > JSHAREVideo) {
    [self.currentContentSupportPlatform addObjectsFromArray:@[@(JSHAREPlatformWechatSession),
                                                              @(JSHAREPlatformWechatFavourite),
                                                              @(JSHAREPlatformWechatTimeLine)]];
  }else if (type > JSHARELink) {
    [self.currentContentSupportPlatform addObjectsFromArray:@[@(JSHAREPlatformWechatSession),
                                                              @(JSHAREPlatformWechatTimeLine),
                                                              @(JSHAREPlatformWechatFavourite),
                                                              @(JSHAREPlatformQQ),
...
- (void)showWithSupportedLoginPlatform{
    [self.currentContentSupportPlatform removeAllObjects];
    [self.currentContentSupportPlatform addObjectsFromArray:@[@(JSHAREPlatformWechatSession),@(JSHAREPlatformQQ),@(JSHAREPlatformSinaWeibo)]];
    [self setShareItem];
    self.hidden = NO;
}
