...
-(void)setSubView{
    ...
    self.title   = [ XYUIKit labelWithBackgroundColor:[UIColor clearColor] textColor:[UIColor blackColor] textAlignment:NSTextAlignmentLeft numberOfLines:1 text:@"推荐课程" fontSize:21];
    self.title.font = FontBold(21);
    self.title.frame = CGRectMake(20, 17, (KScreenWidth-40)/2.0, 29);
    [self.contentView addSubview:self.title];
    
    self.btn = [XYUIKit buttonWithBackgroundColor:[UIColor clearColor] titleColor:mainColor  title:@"全部课程" fontSize:17];
    ...
    self.collecView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 46, KScreenWidth, (KScreenWidth-50)/2.0+62+25) collectionViewLayout:self.flowLayer];
    ...
    UIImageView *line = [[UIImageView alloc]initWithFrame:CGRectMake(0, (KScreenWidth-50)/2.0+62+25+47, KScreenWidth, 1)];
...
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return CGSizeMake((KScreenWidth-50)/2.0,(KScreenWidth-50)/2.0+62);
}
//w+7+48
//点击元素触发事件
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    NSLog(@"%@",indexPath);
    self.blockCell(indexPath);
}
