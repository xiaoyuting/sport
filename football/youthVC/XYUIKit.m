...
//  CateTool
//
//  Created by 雨停 on 2016/12/16.
//  Copyright © 2016年 雨停. All rights reserved.
//

#import "XYUIKit.h"

@implementation XYUIKit
#pragma  mark-----lab-----
+ (UILabel *)labelTextColor:(UIColor *)textColor fontSize:(CGFloat)size{
    return [XYUIKit labelWithBackgroundColor:[UIColor clearColor] textColor:textColor textAlignment:NSTextAlignmentLeft numberOfLines:1 text:nil fontSize:size];
}
...
+ (UIImageView *)imageViewWithImage:(UIImage *)image {
    ...
    return [XYUIKit imageViewWithContentMode:UIViewContentModeScaleToFill userInteractionEnabled:NO image:image];
}

+ (UIImageView *)imageViewWithImage:(UIImage *)image
             userInteractionEnabled:(BOOL)enabled {
    
    return [XYUIKit imageViewWithContentMode:UIViewContentModeScaleToFill userInteractionEnabled:enabled image:image];
}

+ (UIImageView *)imageViewWithContentMode:(UIViewContentMode)mode
                                    image:(UIImage *)image {
    
    return [XYUIKit imageViewWithContentMode:mode userInteractionEnabled:NO image:image];
}
...
+ (UIButton *)buttonWithBackgroundColor:(UIColor *)backgroundColor
    ...
    return [XYUIKit buttonWithBackgroundColor:backgroundColor titleColor:titleColor titleHighlightColor:titleColor title:title fontSize:size];
}
