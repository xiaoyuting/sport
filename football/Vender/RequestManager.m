...
+ (RequestManager *)shareRequestManager
{
    ...
    static id shareRequestManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shareRequestManager = [[super allocWithZone:NULL] init];
    });
...
//        
//        [manager.requestSerializer setValue: [DB getStringById:@"token" fromTable:tabName] forHTTPHeaderField:@"Authorization"];
//    }
        ...
        [imageArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            {   // 图片数据不为空才传递 fileName
                ...
                [formData appendPartWithFileData:imgData
                                            name:@"image"  //[NSString stringWithFormat:@"picflie%ld",(long)idx]
    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status)
        {
            case AFNetworkReachabilityStatusUnknown:
             ...
             RequestManagerShear.netWorkStatu = NetworkStatusUnknown;
                break;
